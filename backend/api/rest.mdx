...
```bash
curl https://nest.owasp.dev/api/v0/projects/ \
...
GET /api/v0/milestones/{milestone_id}
```

Returns paginated GitHub milestones.
...
Returns paginated GitHub organizations.
...
```
GET /api/v0/releases
GET /api/v0/releases/{release_id}
```

Returns paginated GitHub releases.

### Repositories

```
GET /api/v0/repositories
GET /api/v0/repositories/{repository_id}
```

Returns paginated GitHub repositories.
...
```
POST /api/v0/structured-search
```

Accepts a structured query body to filter resources using field-specific operators. See the endpoint's OpenAPI schema for the full request body format.

## Example: fetch projects

```bash
curl "https://nest.owasp.dev/api/v0/projects/?level=flagship&page_size=5" \
  -H "X-Api-Key: your-api-key"
```
